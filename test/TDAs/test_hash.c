...
void hash_pruebas_creacion(){
    test_nuevo_sub_grupo("Pruebas de Creación");
...
void hash_pruebas_en_stack(){
    char clave1[MAX_CLAVE] = "1234";
    int elemento1 = 23;
    hash_t* hash = hash_crear(NULL, 10);
    
    test_nuevo_sub_grupo("Pruebas de inserción en el Stack");
    ...
    test_nuevo_sub_grupo("Pruebas de Borrado en el Stack");
...
void hash_pruebas_en_heap(){
    ...
    test_nuevo_sub_grupo("Pruebas de inserción en Heap(memoria dinámica)");
    ...
    test_nuevo_sub_grupo("Pruebas de Borrado en Heap(memoria dinámica)");
...
void hash_pruebas_casos_limite(){
...
void hash_pruebas_basicas(){
    hash_pruebas_creacion();
    hash_pruebas_en_stack();
    hash_pruebas_en_heap();
}

void hash_pruebas_avanzadas(){
...
bool comparar_vectores_hash(int* vector_1, int* vector_2, size_t tamanio){
    size_t i = 0;
    bool resultado = true;
...
void hash_pruebas_iterador_interno(){
    hash_t* hash = hash_crear(NULL, 10);
    vector_t cargado;
    inicializar_hash_completo(hash, &cargado);
    test_nuevo_sub_grupo("Iterador Interno");
    vector_t interno;
    interno.tope = 0;
    test_afirmar(hash_con_cada_clave(hash, cargar_vector, &interno) == hash_cantidad(hash), "Se recorre todo el hash");
    test_afirmar(comparar_vectores_hash(cargado.vector, interno.vector, 4), "Se pasó por todos los elementos");
...
void hash_pruebas_iterador_externo(){
    hash_t* hash = hash_crear(NULL, 10);
    vector_t cargado;
    inicializar_hash_completo(hash, &cargado);
    test_nuevo_sub_grupo("Iterador Externo");
    vector_t externo;
    externo.tope = 0;
    ...
    test_afirmar(comparar_vectores_hash(cargado.vector, externo.vector, 4), "Se pasó por todos los elementos");
    hash_destruir(hash);
    hash_t* vacio = hash_crear(NULL, 5);
...
/* RECOMPILA TODAS LAS PRUEBAS DE HASH*/
void pruebas_hash(){
    test_nuevo_grupo("PRUEBAS DE HASH");
    hash_pruebas_basicas();
    hash_pruebas_casos_limite();
    hash_pruebas_avanzadas();
    hash_pruebas_iterador_interno();
    hash_pruebas_iterador_externo();
}