...
void heap_pruebas_avanzadas(){
    test_nuevo_sub_grupo("Pruebas avanzadas");
...
void heap_pruebas_basicas(){
    test_nuevo_sub_grupo("Pruebas básicas");
    heap_t* heap = heap_crear(heap_minimal, NULL);
    int numero = 23;
...
void heap_pruebas_creacion(){
    test_nuevo_sub_grupo("Pruebas creación");
...
/* RECOMPILA TODAS LAS PRUEBAS DE HEAP*/
void pruebas_heap(){
    test_nuevo_grupo("PRUEBAS DE HEAP");
    heap_pruebas_creacion();
    heap_pruebas_basicas();
    heap_pruebas_avanzadas();    
}