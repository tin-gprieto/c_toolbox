...
*/
void destructor_dinamico_abb(void* elemento){
    free(elemento);
}
...
*/
bool comparar_vectores_abb(int vector_1[], int recorrido[], size_t tamanio){
    size_t i=0;
    bool coincide=true;
...
void abb_pruebas_iterador(){
    ...
    test_nuevo_sub_grupo("Iteracion interna completa");
    contador.contador = 0;
    ...
    test_nuevo_sub_grupo("Iteracion interna restringida");
    contador.contador = 0;
    ...
    test_nuevo_sub_grupo("Pruebas de fallo con iteración interna");
...
void pruebas_recorrido_completas(size_t (*recorrido)(abb_t*, void**, size_t), int orden_completo[], int orden_incompleto[], int orden_insuficiente[]){
    ...
    test_nuevo_sub_grupo("Recorrido Árbol Completo");
    ...
    orden_adecuado = comparar_vectores_abb(orden_completo, recorrido_completo, cant_recorrido);
    test_afirmar(orden_adecuado, "Se recorrió en el orden correcto");
     
    arbol_destruir(arbol_completo);
    
    test_nuevo_sub_grupo("Recorrido Árbol Incompleto");
    ...
    orden_adecuado = comparar_vectores_abb(orden_incompleto, recorrido_incompleto, cant_recorrido);
    test_afirmar(orden_adecuado, "Se recorrió en el orden correcto");
     
    arbol_destruir(arbol_incompleto);

    test_nuevo_sub_grupo("Recorrido con un array sin capacidad suficiente");
    ...
    print_array(recorrido_incompleto, cant_recorrido);
    orden_adecuado = comparar_vectores_abb(orden_insuficiente, recorrido_insuficiente, tope_insuficiente);
    test_afirmar(orden_adecuado, "Se recorrió en el orden correcto");
     
    arbol_destruir(arbol_insuficiente);
}

void abb_pruebas_recorridos(){

    test_nuevo_sub_grupo("Recorrido Inorden");
    ...
    test_nuevo_sub_grupo("Recorrido Preorden");
    ...
    test_nuevo_sub_grupo("Recorrido Postorden");
...
void prueba_borrado_generica(abb_t* arbol, int* elemento_borrado, size_t cant_inicial, int borrado_esperado[]){
    ...
    test_nuevo_sub_grupo(" Borrado básico");
    ...
    orden_adecuado = comparar_vectores_abb(borrado_esperado, elementos_borrados, cant_inicial-1);
...
void notificar_elemento_borrado(int i){
    printf("    Borra el elemento : %i \n", i);
}

void abb_pruebas_borrado(){
    abb_t * arbol = arbol_crear(comparador_int, NULL);
    //                 0  1  2  3  4  5  6  7  8 
    ...
    test_nuevo_sub_grupo("Borrar elemento fuera de árbol");
    ...
    bool orden_adecuado = comparar_vectores_abb(elementos, recorrido_int, tope);
    test_afirmar(orden_adecuado, "Se conserva el orden de los elementos"); 

    test_nuevo_sub_grupo("Borrar con Predecesor inorden (hoja)");
    int borrado_1_esperado[] = {5, 3, 2, 4, 8, 7, 9, 12};
    printf("    Árbol(preorden): ");
    print_array(elementos, tope);
    notificar_elemento_borrado(elementos[0]); //6
    prueba_borrado_generica(arbol, &elementos[0], tope, borrado_1_esperado);
    tope --;

    test_nuevo_sub_grupo("Borrar sin Predecesor menor (reemplaza hijo mayor)");
    int borrado_2_esperado[] = {5, 3, 2, 4, 8, 7, 12};
    notificar_elemento_borrado( elementos[7]); //9
    prueba_borrado_generica(arbol, &elementos[7], tope, borrado_2_esperado);
    tope --;
    
    test_nuevo_sub_grupo("Borrar con Predecesor inmediato hoja");
    int borrado_3_esperado[] = {5, 3, 2, 4, 7, 12};
    notificar_elemento_borrado( elementos[5]); //8
    prueba_borrado_generica(arbol, &elementos[5], tope, borrado_3_esperado);
    tope --;
    
    test_nuevo_sub_grupo("Borrar un elemento sin hijos");
    int borrado_4_esperado[] = {5, 3, 2, 4, 7}; //12
    notificar_elemento_borrado( elementos[8]);
    prueba_borrado_generica(arbol, &elementos[8], tope, borrado_4_esperado);
    tope --;

    test_nuevo_sub_grupo("Borrar una raíz con Predecesor inorden (hijo del inmediato)");
    int borrado_5_esperado[] = {4, 3, 2, 7};
    notificar_elemento_borrado( elementos[4]); //5
    prueba_borrado_generica(arbol, &elementos[4], tope, borrado_5_esperado);
    tope --;

    test_nuevo_sub_grupo("Borrar una raíz con Predecesor inmediato (con hijo)");
    int borrado_6_esperado[] = {3, 2, 7}; 
    notificar_elemento_borrado( elementos[3]); //4
    prueba_borrado_generica(arbol, &elementos[3], tope, borrado_6_esperado);
    tope --;

    test_nuevo_sub_grupo("Borrar una raíz de dos hijos");
    int borrado_7_esperado[] = {3, 7}; 
    notificar_elemento_borrado( elementos[2]); //2
    prueba_borrado_generica(arbol, &elementos[2], tope, borrado_7_esperado);
    tope --;

    test_nuevo_sub_grupo("Borrar una raíz sin Predecesor menor");
    int borrado_8_esperado[] = {7}; 
    notificar_elemento_borrado( elementos[1]); //3
    prueba_borrado_generica(arbol, &elementos[1], tope, borrado_8_esperado);
    tope --;

    test_nuevo_sub_grupo("Borrar una raíz Hoja");
    int borrado_9_esperado[tope];
    notificar_elemento_borrado( elementos[6]); //7
...
void abb_pruebas_busqueda(){
    test_nuevo_sub_grupo("Pruebas con Buscar");
    abb_t *arbol = arbol_crear(comparador_int, NULL);
...
void abb_pruebas_insercion(){
    ...
    bool estado = comparar_vectores_abb(elementos, elementos_recorridos, tope);
    test_afirmar(estado, "El árbol se insertó en el orden correcto");   

    arbol_destruir(arbol_2);
}

void abb_pruebas_basicas(){

    test_nuevo_sub_grupo("Creación del árbol");
    abb_t* arbol = arbol_crear(comparador_int, destructor_dinamico_abb);
    ...
    arbol_destruir(arbol);
    arbol = arbol_crear(NULL, destructor_dinamico_abb);
...
/* RECOMPILA TODAS LAS PRUEBAS DE ABB*/
void pruebas_abb(){
    test_nuevo_grupo("PRUEBAS DE ABB");
    abb_pruebas_basicas(); //Crear, Vacio, Destruir, Raiz   
    abb_pruebas_insercion(); //Insertar, Raiz
    abb_pruebas_borrado();
    abb_pruebas_busqueda(); 
    abb_pruebas_recorridos();
    abb_pruebas_iterador();
}